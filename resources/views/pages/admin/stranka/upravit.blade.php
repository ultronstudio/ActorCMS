...
@section('content')
    <div class="row mt-4">
        ...
        <hr>
        <div class="">
            <form method="POST" action="{{ url('/admin/stranka/upravit/' . $page->id) }}" id="formular-upravit_prispevek"
                enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label for="nazev" class="form-label">Název</label>
                    <input type="text" id="stranka_nazev" name="title" value="{{ $page->title }}"
                        class="form-control" required
                        oninput="adminPanel.createPostSlug(document.getElementById('stranka_nazev'), document.getElementById('stranka_slug'))">
                </div>
                <div class="mb-3">
                    <label for="slug" class="form-label">Slug</label>
                    <input type="text" id="stranka_slug" name="slug" value="{{ $page->slug }}"
                        class="form-control" required readonly>
                </div>
                <div class="mb-3">
                    <label for="type" class="form-label">Obsah</label>
                    <textarea name="content" id="obsah" cols="30" rows="10" class="form-control">{{ $page->content }}</textarea>
                </div>
                <div class="mb-3">
